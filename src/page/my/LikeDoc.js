...
class LikeDoc extends Component {
  constructor(props) {
    super(props);
    this._getDocList = this._getDocList.bind(this);
  }
  componentDidMount() {
    let {likeDoc = {}} = this.props;
    if (likeDoc.list.length === 0) {
      this._getDocList(1);
    }
  }
  back() {
  ...
  _getDocList(page) {
    this.props.dispatch(getUserDoc({docViewType: 2, page}));
  }
  ...
  _onEndReached(info) {
    ...
    if (info.distanceFromEnd > 0) {
      let { likeDoc } = this.props;
      let dataPage = likeDoc.page + 1;
      if (dataPage <= likeDoc.maxPage) {
  ...
  render() {
    let {likeDoc = {}, refreshing, loadedData} = this.props;
    let { page, maxPage, list = [] } = likeDoc;
    return (
...
export default connect(select)(LikeDoc);
