...
class SignUp extends Component {
  ...
  _signUpCallback () {
    ...
    Keyboard.dismiss();
    ModalIndicator.show('正在注册...');
    this.props.dispatch(register({username, password: MD5(password), code}));
  }
  async _getCode () {
    let {username} = this;
    if (username == null || username === '') {
    ...
    let handle = await sendCode({phone: username, smsId: 'SMS_118620008'});
    if (handle) {
      ...
      <View style={styles.view}>
        ...
        <View style={styles.editGroup}>
          ...
          <View style={{marginTop: px2dp(15), height: px2dp(40)}}>
            <Button text='注册' onPress={this._signUpCallback} />
          </View>
        </View>
      </View>
    );
